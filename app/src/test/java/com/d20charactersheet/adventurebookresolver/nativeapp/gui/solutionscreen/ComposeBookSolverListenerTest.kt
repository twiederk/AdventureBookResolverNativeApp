...
class ComposeBookSolverListenerTest {
    ...
    private val solutionScreenViewModel: SolutionScreenViewModel = mock()
    private val logger: Logger = mock()
    private val composeBookSolverListener =
        ComposeBookSolverListener(solutionScreenViewModel, logger)
    ...
    fun `should update number of remaining combinations and write it to log when calculateCombinations is fired`() {
        ...
        // assert
        verify(solutionScreenViewModel).onRemainingCombinationsChange(10)
    }

    @Test
    fun `should update max combinations in SolutionScreenViewModel`() {

        // act
        composeBookSolverListener.maxCombinations(100)

        // assert
        verify(solutionScreenViewModel).onMaxCombinationsChange(100)
    }
    ...
    fun `should update number of solutions`() {
        ...
        // assert
        verify(solutionScreenViewModel).onSolutionFoundChange(1)
    }
